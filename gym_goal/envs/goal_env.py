...
class GoalEnv(gym.Env):
    ...
    def _update(self, act, param):
        ...
        self.states.append([
            ...
            self.goalie2.position.copy(),
            self.goalie2.orientation,
            self.goalie3.position.copy(),
            self.goalie3.orientation,
            self.player2.position.copy(),
            self.player2.orientation])
    ...
    def reset(self):
        ...
        initial_player2 = np.array((0, self.np_random.uniform(-PITCH_WIDTH / 2, PITCH_WIDTH / 2)))
        angle = angle_between(initial_player, np.array((PITCH_LENGTH / 2, 0)))
        angle2 = angle_between(initial_player, np.array((PITCH_LENGTH / 2, 0)))
        self.player = Player(initial_player, angle)
        self.player2 = Player(initial_player2, angle2)
        ...
        initial_goalie2 = self._keeper_target(initial_ball+np.array([1,1]))
        initial_goalie3 = self._keeper_target(initial_ball+np.array([-1,-1]))

        angle2 = angle_between(initial_goalie2, initial_ball)
        angle3 = angle_between(initial_goalie3, initial_ball)
        self.goalie = Goalie(initial_goalie, angle2)
        self.goalie2 = Goalie(initial_goalie2, angle2)
        self.goalie3 = Goalie(initial_goalie3, angle3)

        self.entities = [self.player, self.goalie, self.ball, self.goalie2, self.goalie3, self.player2]
    ...
    def __draw_internal_state(self, internal_state, fade=False):
        ...
        goalie2_position = internal_state[5]
        goalie2_orientation = internal_state[6]
        goalie3_position = internal_state[7]
        goalie3_orientation = internal_state[8]
        player2_position = internal_state[9]
        player2_orientation = internal_state[10]
        ...
        self.__draw_player(player2_position, player2_orientation, self.__white)
        if not fade:
            self.__draw_radius(player_position, KICKABLE)
            self.__draw_radius(player2_position, KICKABLE)

        self.__draw_player(goalie_position, goalie_orientation, self.__red)
        self.__draw_player(goalie2_position, goalie2_orientation, self.__red)
        self.__draw_player(goalie3_position, goalie3_orientation, self.__red)

        if not fade:
            self.__draw_radius(goalie_position, CATCHABLE)
            self.__draw_radius(goalie2_position, CATCHABLE)
            self.__draw_radius(goalie3_position, CATCHABLE)
        # Draw ball
